...
#include <msf_updates/PositionPoseMagSensorConfig.h>

 namespace msf_updates {

  typedef msf_updates::PositionPoseMagSensorConfig Config_T;
  typedef dynamic_reconfigure::Server<Config_T> ReconfigureServer;
  typedef shared_ptr<ReconfigureServer> ReconfigureServerPtr;

  class PositionPoseMagSensorManager : public msf_core::MSF_SensorManagerROS<
  ...
  public:
    ...
    PositionPoseMagSensorManager(
      ros::NodeHandle pnh = ros::NodeHandle("~/position_pose_mag_sensor")) {
      imu_handler_.reset(
    ...
    pose_handler_.reset(
      new PoseSensorHandler_T(*this, "", "vision_sensor", distortmeas));
  ...
  virtual ~PositionPoseMagSensorManager() {
  }
...
private:
   ...
   */
   virtual void Config(Config_T &config, uint32_t level) {
    ...
    mag_handler_->SetNoises(config.mag_noise_meas_p,
     config.mag_noise_meas_q);
    ...
    if((level & msf_updates::PositionPoseMagSensor_SET_LATLON) && config.reset_coordinate == true)
    {
    ...
    if ((level & msf_updates::PositionPoseMagSensor_INIT_FILTER)
  ...
  if ((level & msf_updates::PositionPoseMagSensor_SET_HEIGHT)
...
void Init(double scale) const {
  ...
  p_ip, p_pos;
  Eigen::Quaternion<double> q, q_wv, q_ic, q_vc,q_mag;
  msf_core::MSF_Core<EKFState_T>::ErrorStateCov P;
    ...
    q_mag = mag_handler_->GetAttitudeMeasurement();
    MSF_INFO_STREAM(
    ...
    if (!mag_handler_->ReceivedFirstMeasurement())
      MSF_WARN_STREAM(
        "No measurements received yet to initialize mag attitude - "
        "using [1 0 0 0] respectively");

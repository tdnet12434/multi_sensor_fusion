 ...
 */
#ifndef VELOCITY_POSE_SENSOR_MANAGER_H
#define VELOCITY_POSE_SENSOR_MANAGER_H
...
#include <msf_updates/VelocityPoseSensorConfig.h>

 namespace msf_updates {

  typedef msf_updates::VelocityPoseSensorConfig Config_T;
  typedef dynamic_reconfigure::Server<Config_T> ReconfigureServer;
  typedef shared_ptr<ReconfigureServer> ReconfigureServerPtr;

  class VelocityPoseSensorManager : public msf_core::MSF_SensorManagerROS<
  msf_updates::EKFState> {
    typedef VelocityPoseSensorManager this_T;
  ...
  public:
    ...
    VelocityPoseSensorManager(
      ros::NodeHandle pnh = ros::NodeHandle("~/velocity_pose_sensor")) {
      imu_handler_.reset(
  ...
  virtual ~VelocityPoseSensorManager() {
  }
...
private:
   ...
   */
   virtual void Config(Config_T &config, uint32_t level) {
    ...
    velocity_handler_->SetDrag(config.velocity_drag);


    
  if ((level & msf_updates::VelocityPoseSensor_INIT_FILTER)
  ...
  if ((level & msf_updates::VelocityPoseSensor_SET_HEIGHT)
...
void Init(double scale) const {
  ...
  msf_core::MSF_Core<EKFState_T>::ErrorStateCov P;
    ...
    pnh.param("velocity_sensor/init/p_ip/x", p_ip[0], 0.0);
    pnh.param("velocity_sensor/init/p_ip/y", p_ip[1], 0.0);
    pnh.param("velocity_sensor/init/p_ip/z", p_ip[2], 0.0);
    ...
    * p_vc / scale - q.toRotationMatrix() * p_ic;
    ...
    // measurement.
    p = p_vision;//p_pos - q.toRotationMatrix() * p_ip;
...
#endif  // VELOCITY_POSE_SENSOR_MANAGER_H

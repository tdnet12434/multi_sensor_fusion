...
struct AglMeasurement : public AglMeasurementBase {
 private:
  ...
  typedef Measurement_t::Measurement_ptr measptr_t;
    ...
    z_sonar_ = Eigen::Matrix<double,1,1>::Constant(msg->range);
    printf("sonar = %.2f\n", z_sonar_(0));
 ...
 public:
  ...
  AglMeasurement(double n_zsonar, bool fixed_covariance,
  ...
  virtual void CalculateH(
    ...
    if (z_sonar_(0) < 0.3 || z_sonar_(0) > 4.0) {
      sonar_healhy = false;
    }else {
      sonar_healhy = true;
    }

    z_sonar_compensated = z_sonar_ * eff;
